"""
Build Anki .apkg from vocabulary markdown file.
"""
import re
import sys
import genanki

INPUT_PATH = r'E:\UTILS\extractor\nihongo_practices\vocabulary\1.txt'
OUTPUT_PATH = r'E:\UTILS\extractor\nihongo_practices\vocabulary\1.apkg'
...
def extract_image_urls(text: str) -> str:
    """Convert markdown image links to simple <img> tags."""
    # Pattern for [url](url) or [text](url)
    def repl(m):
        url = m.group(2).strip()
        # simple heuristic: if url contains image-like path, use img tag
...
def highlight_kanji_in_question(text: str) -> str:
    """Keep <u> as simple red highlight (allowed exception for target Kanji)."""
    # Replace <u>...</u> with a colored span (not bold/italic/underline)
...
def clean_explanation(text: str) -> str:
    ...
    # Convert bullet `* ` to `- ` for cleaner look, but keep structure
    lines = []
...
def parse_input(path: str):
    ...
    # Deck name from first heading line
    first_line = text.splitlines()[0].strip()
    deck_name = first_line.lstrip('#').strip()

    # Split question part / explanation part at the first occurrence of
    #   # 問題1 — ...
    # This separates the question blocks from the detailed explanations.
    ...
    if split_match:
        ...
        # fallback: if no explanation heading, just use whole text as questions
        question_text = text
        explanation_text = ""
    ...
    for m in q_pattern.finditer(question_text):
        ...
        for om in opt_pattern.finditer(body):
            ...
            opts[idx] = val
        # Question text = everything before first "1. "
        first_opt = body.find('1. ')
        if first_opt != -1:
            q_str = body[:first_opt].strip()
        else:
            q_str = body
        # Process images and kanji highlight in question
    ...
    # Extract answer number from explanation body
    for num, e in explanations.items():
        m = re.search(r'\*\*Đáp án:\s*(\d)\.', e['body'])
        if m:
